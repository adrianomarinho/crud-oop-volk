 ...
 * About Database\Orm
 *
 * Database\Orm is a micro-ORM which functions as both a fluent select query API and a CRUD model class.
 * Database\Orm is built on top of PDO and is well fit for small to mid-sized projects, where the emphasis
 * is on simplicity and rapid development rather than infinite flexibility and features.
 * Database\Orm works easily with table relationship.
 *
 *
 */
use \ArrayIterator;
...
class Orm implements \IteratorAggregate
{
    ...
    public function where($condition, $parameters = [])
    {
        ...
        } elseif (!is_array($parameters)) {//where(colum,value) => colum=value
            $condition .= " = ?";
            $parameters = [$parameters];
        } elseif (is_array($parameters)) { // where("column", array(1, 2)) => column IN (?,?)
